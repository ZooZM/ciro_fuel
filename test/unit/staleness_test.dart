...
void main() {
  ...
  test('a sample within the stale window is not stale', () {
    ...
    final now = recordedAt.add(
      AppDurations.locationStaleWindow - const Duration(seconds: 1),
    );
  ...
  test(
    'staleness is measured from receivedAt (server stamp) when present, not recordedAt',
    () {
  ...
  test(
    'falls back to recordedAt when the server did not include receivedAt',
    () {
      final sample = LocationSample(
        lat: 24.7136,
        lng: 46.6753,
      ...
      final now = recordedAt.add(
        AppDurations.locationStaleWindow + const Duration(minutes: 1),
      );
